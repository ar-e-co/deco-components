...
async function simulateShipping(
  ...
  try {
    ...
    const { cheapestOption, fastestOption, pickupOptions = [] } =
      aggregateSimulationResults(results?.logisticsInfo ?? []) ?? {};
    ...
    if (!cheapestOption) {
      ...
      return slas.map((sla) => ({
        id: sla.id,
        friendlyName: sla.deliveryChannel === SelectedDeliveryChannel.Delivery
          ? sla.name
    ...
    const pickup: ShippingSimulationSLA[] = pickupOptions.map((option) => ({
      ...
      price: option.price,
      pickupOptions: option.locations?.map((location) =>
